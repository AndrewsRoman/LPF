...
int queue (fila_t* f, int elemento){
	...
	if(fila_vazia(f)){
		...
		f->fim->prox=temp; /*proximo elemento depois do final passa a ser o nó*/
	}
	/* final aponta para o temp */
	f->fim = temp; /*novo final da fila*/
	f->tamanho++;
	return 1;
}
...
int dequeue (fila_t* f, int* elemento){
	...
	f->tamanho--;
	*elemento= temp->chave;
	if(f->ini == NULL){	/*se o inicio for NULL a fila estava vazia e atribui NULL para f->fim*/
		f->fim= NULL;
	}
	free(temp);
	return 1;
