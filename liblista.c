...
int lista_insere_fim (lista_t* l, int elemento){
	...
	nodo_l_t* aux;
	aux = l->ini;
	/* se a lista não for vazia percorre a lista ate o final e insere o nodo */
	while(aux->prox != NULL){
...
int lista_retira_fim (lista_t* l, int* elemento){
	...
	while(temp->prox!=NULL){
		anterior = temp; /*anterior recebe novo no*/
		temp = temp->prox; /*o nó recebe o proximo elemento*/
	}
	*elemento = temp->elemento;
	if(temp == l->ini) /*se a lista só tiver um elemento*/
		l->ini = temp->prox;
	else
		anterior->prox=temp->prox; /*anterior passa a apontar para onde o no aponta*/
	l->tamanho--;
	free(temp);
...
int lista_retira_elemento (lista_t *l, int *elemento){
	...
	while(cont < l->tamanho){
		if (temp->elemento == *elemento){
			...
			if (temp==l->ini){ /*se for no inicio da lista*/
				l->ini=temp->prox; /*inicio recebe o proximo no*/
				free(temp);
				l->tamanho--;
				return 1;
			}
